angular.module('blogAdmin').controller('FileManagerController', ["$rootScope", "$scope", "$location", "$filter", "$log", "dataService", function ($rootScope, $scope, $location, $filter, $log, dataService) {
    ...
    $scope.processChecked = function (action) {
        ...
        if (action === "append") {
            ...
            while (j--) {
                ...
                if (item.FileType === 1) {
                    var fileTag = "<p><a href='" + SiteVars.RelativeWebRoot + "res/file?file=" + item.FullPath + "' target='_blank'>" + item.Name + " (" + item.FileSize + ")</a></p>";
                    editorSetHtml(editorHtml + fileTag);
                }
                if (item.FileType === 2) {
                    editorSetHtml(editorHtml + "<img src='" + SiteVars.RelativeWebRoot + "res/image?picture=" + item.FullPath + "' />");
                }
            }
    ...
    $scope.insertFile = function (file) {
        var s = "![" + file.Name + "](" + SiteVars.RelativeWebRoot + "res/image?picture=" + file.FullPath + ")";
        if (file.FileType != 2) {
            // not a picture, insert as attachement
            //s = "<p><a class=\"download\" href=\"" + SiteVars.RelativeWebRoot + "res/file?file=" +
            //    file.FullPath + "\" alt=\"" + file.Name + "\">" + file.Name + " (" + file.FileSize + ")</a></p>";
            s = "[" + file.Name + "(" + file.FileSize + ")](" + SiteVars.RelativeWebRoot + "res/file?file=" + file.FullPath + ")";
        }
        // get hold on TinyMce editor and inject link
        var editor = $('#md-editor-input', top.document);
